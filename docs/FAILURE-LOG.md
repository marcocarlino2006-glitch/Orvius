...
| 2026-09-04 | Stripe SaaS | No STRIPE_* in agent `.env` | High | **Closed on prod** — `/api/billing/checkout` `configured:true` (2026-09-23); agent `.env` still empty; self-serve stays off until public launch ready |
| 2026-09-23 | Gate honesty | Agent `life:check` / Manus painted Twilio paste while prod already `configured:true` | High | **Fixed in product** — prod health probe for telephony; local empty ≠ prod dark |
| 2026-09-23 | Gate honesty | Agent painted Stripe paste while prod billing `configured:true` | High | **Fixed in product** — prod billing probe; Manus NEXT skips to formation when local theater |
| 2026-09-23 | Institutional | Orphan `src/lib/trust.ts` (Summit case-study theater) + 46 dead CSS classes | Medium | **Fixed** — deleted orphan; `css:prune --write` |
...
| 2026-09-05 | Email backup | No RESEND_API_KEY / Summit ownerEmail was null | High | **Partial** — ownerEmail set; **founder** paste RESEND_API_KEY (`npm run founder:next`) |
