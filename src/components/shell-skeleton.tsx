...
export function SkeletonLeadCard() {
  return (
    <div className="lead-rail-row dashboard-list-skeleton-row" aria-hidden>
      <div className="lead-rail-main">
        <div className="lead-rail-meta">
        <SkeletonBar className="h-3 w-24" />
        <SkeletonBar className="h-3 w-16" />
      </div>
        <SkeletonBar wide className="mt-3 h-5 max-w-[12rem]" />
...
export function DashboardSkeleton() {
  return (
    <div aria-busy="true" aria-label="Loading dashboard">
      <ul className="os-lead-rail dashboard-list-skeleton">
        {[1, 2, 3, 4, 5].map((item) => (
          <li key={item}>
          <SkeletonLeadCard />
          </li>
